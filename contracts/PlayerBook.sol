...
contract PlayerBook is OwnableUpgradeable {
    ...
    address private constant NAME_FEE_ADDRESS =
        0x6Aa9A4aaf122a440b5eA3036C020E562327A85Ed;
    ...
    function multiSigDev(bytes32 _whatFunction) private returns (bool) {
        return (
            MSFun.multiSig(msData, Team.requiredDevSignatures(), _whatFunction)
        );
    }
    ...
    function checkSignersByAddress(
    ...
    {
        return (
            Team.adminName(MSFun.checkSigner(msData, _whatFunction, _signerA)),
            Team.adminName(MSFun.checkSigner(msData, _whatFunction, _signerB)),
            Team.adminName(MSFun.checkSigner(msData, _whatFunction, _signerC))
        );
    }
    ...
    //    (_|(_| | (_|  _\(/_ | |_||_)  .
    //=============================|================================================
    uint256 public registrationFee_; // price to register a name
    ...
    //==============================================================================
    function initialize(address _team) external initializer {
        __Ownable_init();
        require(owner() != address(0), "owner must be set");
        // premine the dev names (sorry not sorry)
        // No keys are purchased with this method, it's simply locking our addresses,
        // PID's and names for referral codes.
        plyr_[1].addr = 0x6Aa9A4aaf122a440b5eA3036C020E562327A85Ed;
        plyr_[1].name = "deployer";
        plyr_[1].names = 1;
        pIDxAddr_[0x6Aa9A4aaf122a440b5eA3036C020E562327A85Ed] = 1;
    ...
    function addMeToGame(uint256 _gameID) public isHuman() {
        ...
        if (_totalNames > 1)
            for (uint256 ii = 1; ii <= _totalNames; ii++)
                games_[_gameID].receivePlayerNameList(
                    _pID,
                    plyrNameList_[_pID][ii]
                );
    }
    ...
    function addMeToAllGames() public isHuman() {
        ...
        for (uint256 i = 1; i <= gID_; i++) {
            ...
            if (_totalNames > 1)
                for (uint256 ii = 1; ii <= _totalNames; ii++)
                    games_[i].receivePlayerNameList(
                        _pID,
                        plyrNameList_[_pID][ii]
                    );
        }
     ...
     */
    function useMyOldName(string memory _nameString) public isHuman() {
    ...
    function recordReferralCommission(address _referrer, uint256 _commission) public isRegisteredGame {
    ...
    function registerNameCore(
        ...
        // registration fee
        payable(NAME_FEE_ADDRESS).transfer(address(this).balance);
     ...
     * @param _name player desired name
     * @param _affCode referral code
     * @param _all set to true if you want this to push your info to all games
     * (this might cost a lot of gas)
     */
    ...
    function setRegistrationFee(uint256 _fee) public onlyDevs() {
